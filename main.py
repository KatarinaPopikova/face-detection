...
if __name__ == '__main__':
    ...
    path = 'data/viz_vzorka/'
    files = os.listdir(path)

    header = ['name', 'TP', 'FP', 'FN', 'precision', 'recall', 'mse']
    csv_path = 'data/saveData_mtcnn_mse'
    ...
    with open(csv_path + '.csv', 'w', encoding='UTF8', newline='') as f:
        ...
        for file in files:
            ...
            colorImg_original = data['colorImages_original']
            colorImg_medium = data['colorImages_medium']
            colorImg_serve = data['colorImages_severe']
            boundingBox = data['boundingBox']
            landmarks2D = data['landmarks2D']

            load_save_video('data/output/mtcnn/' + file[:-4] + '/original', colorImg_original, boundingBox, landmarks2D,
                            writer)
            load_save_video('data/output/mtcnn/' + file[:-4] + '/medium', colorImg_medium, boundingBox, landmarks2D,
                            writer)
            load_save_video('data/output/mtcnn/' + file[:-4] + '/severe', colorImg_serve, boundingBox, landmarks2D,
                            writer)
